...
```text
neural_rx/scripts/temporal_nrx_runtime.py
```
...
The stock srsRAN source tree in this project does not itself contain the NVIDIA TensorFlow/TensorRT neural receiver implementation or an existing native signal adapter that converts the live PUSCH processing buffers into the `received_grid` and `ls_estimate` tensors expected by `TemporalUEMemoryCGNN`.
